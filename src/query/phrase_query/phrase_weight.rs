...
impl PhraseWeight {
    ...
    fn fieldnorm_reader(&self, reader: &SegmentReader) -> crate::Result<FieldNormReader> {
        ...
        if self.is_fieldnorms_scoring_enabled && self.similarity_weight_opt.is_some() {
    ...
    pub(crate) async fn phrase_scorer_async(
        ...
        let similarity_weight_opt = self
            ...
            .map(|similarity_weight| similarity_weight.boost_by(boost));
        let fieldnorm_reader = self.fieldnorm_reader(reader)?;
        let term_postings_list = if reader.has_deletes() {
            join_all(self.phrase_terms.iter().map(|(offset, term)| async move {
                if let Some(postings) = reader
                    .inverted_index_async(term.field())
                    .await?
                    .read_postings_async(term, IndexRecordOption::WithFreqsAndPositions)
                    .await?
                {
                    Ok(Some((*offset, postings)))
                } else {
                    Ok(None)
                }
            }))
        ...
        if term_postings_list.len() < self.phrase_terms.len() {
            return Ok(None);
        }
        Ok(Some(PhraseScorer::new(
            term_postings_list,
