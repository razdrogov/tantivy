...
pub struct TermInfo {
    ...
    pub postings_range: Range<u64>,
    /// Byte range of the positions of this terms in the positions (`.pos`) file.
    pub positions_range: Range<u64>,
}

impl TermInfo {
    pub(crate) fn posting_num_bytes(&self) -> u32 {
        let num_bytes = self.postings_range.end - self.postings_range.start;
        assert!(num_bytes <= u32::MAX as u64);
        num_bytes as u32
    }

    pub(crate) fn positions_num_bytes(&self) -> u32 {
        let num_bytes = self.positions_range.end - self.positions_range.start;
        assert!(num_bytes <= u32::MAX as u64);
        num_bytes as u32
    }
}
...
impl BinarySerializable for TermInfo {
    ...
    fn deserialize<R: io::Read>(reader: &mut R) -> io::Result<Self> {
        ...
        let postings_num_bytes = u32::deserialize(reader)? as u64;
        let postings_end_offset = postings_start_offset + postings_num_bytes;
        let positions_start_offset = u64::deserialize(reader)?;
        let positions_num_bytes = u32::deserialize(reader)? as u64;
